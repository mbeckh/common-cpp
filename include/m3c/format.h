...
struct BaseFormatter {  // NOLINT(readability-identifier-naming): Windows/COM naming convention.
	...
	constexpr fmt::format_parse_context::iterator parse(const fmt::format_parse_context& ctx) {
		auto it = ctx.begin();
		...
		if (end != it) {
			...
			m_format.assign("{:").append(it, end).push_back('}');
		}
		return end;
	}

protected:
	[[nodiscard]] constexpr const std::string& GetFormat() const noexcept {  // NOLINT(readability-identifier-naming): Windows/COM naming convention.
		return m_format;
	}
...
template <>
struct fmt::formatter<GUID> : public m3c::internal::BaseFormatter {
	/// @brief Format the `GUID`.
	/// @param arg A `GUID`.
	/// @param ctx see `fmt::formatter::format`.
	/// @return see `fmt::formatter::format`.
	fmt::format_context::iterator format(const GUID& arg, fmt::format_context& ctx);
};

template <>
struct fmt::formatter<FILETIME> : public m3c::internal::BaseFormatter {
	fmt::format_context::iterator format(const FILETIME& arg, fmt::format_context& ctx);
};

template <>
struct fmt::formatter<SYSTEMTIME> : public m3c::internal::BaseFormatter {
	fmt::format_context::iterator format(const SYSTEMTIME&, fmt::format_context& ctx);
};

template <>
struct fmt::formatter<SID> : public m3c::internal::BaseFormatter {
	fmt::format_context::iterator format(const SID&, fmt::format_context& ctx);
};


template <>
struct fmt::formatter<m3c::internal::win32_error> : public m3c::internal::BaseFormatter {
	fmt::format_context::iterator format(const m3c::internal::win32_error&, fmt::format_context& ctx);
};

template <>
struct fmt::formatter<m3c::internal::rpc_status> : public m3c::internal::BaseFormatter {
	fmt::format_context::iterator format(const m3c::internal::rpc_status&, fmt::format_context& ctx);
};

template <>
struct fmt::formatter<m3c::internal::hresult> : public m3c::internal::BaseFormatter {
	fmt::format_context::iterator format(const m3c::internal::hresult&, fmt::format_context& ctx);
};

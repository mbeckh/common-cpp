...
static std::ostream& operator<<(std::ostream& os, const Priority priority) {
	...
	case Priority::kTrace:
		...
		break;
	default:
		throw std::out_of_range(fmt::format("priority {}", priority));
	}
	return os << name;
}
...
static std::ostream& operator<<(std::ostream& os, const EventType eventType) {
	std::string name;
	switch (eventType) {
	...
	case EventType::kStringRef:
		...
		break;
	default:
		throw std::out_of_range(fmt::format("eventType {}", eventType));
	}
	return os << name;
}
...
protected:
	std::string GetLogEventOutput(const char* const message, const char* const dynamicLogMessage, const m3c::source_location& loc, const std::uint32_t line) {
		return fmt::format("[{}] [{}] {}{}\n\tat {}({}) ({})\n", GetLevelOutput(), GetCurrentThreadId(), message, dynamicLogMessage, loc.file_name(), line, loc.function_name());
	}

	std::string GetExceptionOutput(const char* const message, const m3c::source_location& loc, const std::uint32_t line) {
		return fmt::format(std::string("\tcaused by: {}\n") + (line ? "\t\tat {}({}) ({})\n" : ""), message, loc.file_name(), line, loc.function_name());
	}
	...
	auto TestMethod(const char* const dynamicLogMessage = "", E&& exception = 0, const char* const exceptionMessage = nullptr, const D detail = 0, Args&&... args) {
		...
		EXPECT_CALL(m_win32, OutputDebugStringA(t::StartsWith("[")))
			.Times(kOutputDebug && IsLogged() ? 1 : 0);

		EXPECT_CALL(m_win32, fputs(m4t::MatchesRegex("^\\d{4}-[\\s\\S]*"), stderr))
			.Times(kOutputStderr && IsLogged() ? 1 : 0);

		EXPECT_CALL(m_win32, EventWriteEx(DTGM_ARG8))
			.Times(0);
		if constexpr (kOutputEvent) {
			if (eventType == EventType::kEvent) {
				...
				EXPECT_CALL(m_win32, EventWriteEx(t::_,
				                                  t::AllOf(t::Field(&EVENT_DESCRIPTOR::Id, !kHRESULT ? evt::TestEvent0.Id : evt::TestEvent2.Id),
				                                           t::Field(&EVENT_DESCRIPTOR::Level, static_cast<std::underlying_type_t<Priority>>(GetLevel())),
				                                           t::Field(&EVENT_DESCRIPTOR::Keyword, t::Not(m4t::BitsSet(M3C_KEYWORD_EXCEPTION)))),
				                                  DTGM_ARG4, !kHRESULT ? 1 : 2, t::_))
					.Times(IsLogged() ? 1 : 0)
					.InSequence(seq);
			}
		}
		...
		std::uint32_t line = 0;
		source_location exceptionLocation;
		source_location innerExceptionLocation;
		...
		if constexpr (!kException) {
			m4t::WithLocale("en-US", [eventType, level = GetLevel(), &line, &location] {
				switch (eventType) {
				case EventType::kEvent:
					line = (location = source_location::current()).line() + 1;
					Log::Event(level, evt::TestEvent0, "mymessage");
					break;
				case EventType::kCharPtr:
					line = (location = source_location::current()).line() + 1;
					Log::Event(level, "zstring event 0: {}", "mymessage");
					break;
				case EventType::kString:
					line = (location = source_location::current()).line() + 1;
					Log::Event(level, std::string("string event 0: {}"), "mymessage");
					break;
				case EventType::kStringRef: {
					const std::string str = "string& event 0: {}";
					line = (location = source_location::current()).line() + 1;
					Log::Event(level, str, "mymessage");
					break;
				}
				default:
					throw std::out_of_range(fmt::format("eventType {}", eventType));
			...
			try {
				if constexpr (!kDetail) {
					...
					if constexpr (!kNested) {
						exceptionLine = (exceptionLocation = source_location::current()).line() + 1;
						throw((std::move(exception) + detail) << ... << std::forward<Args>(args));
					} else {
						try {
							innerExceptionLine = (innerExceptionLocation = source_location::current()).line() + 1;
							throw std::exception("inner cause") + evt::Default;
						} catch (...) {
							exceptionLine = (exceptionLocation = source_location::current()).line() + 1;
							std::throw_with_nested(((std::move(exception) + detail) << ... << std::forward<Args>(args)));
						}
					}
				}
			} catch (...) {
				m4t::WithLocale("en-US", [eventType, level = GetLevel(), &line, &location, &hr] {
					if constexpr (!kHRESULT) {
						...
						case EventType::kEvent:
							line = (location = source_location::current()).line() + 1;
							Log::Exception(level, evt::TestEvent0, "mymessage");
							break;
						case EventType::kCharPtr:
							line = (location = source_location::current()).line() + 1;
							Log::Exception(level, "zstring event 0: {}", "mymessage");
							break;
						case EventType::kString:
							line = (location = source_location::current()).line() + 1;
							Log::Exception(level, "string event 0: {}", "mymessage");
							break;
						case EventType::kStringRef: {
							const std::string str = "string& event 0: {}";
							line = (location = source_location::current()).line() + 1;
						...
						case EventType::kEvent:
							line = (location = source_location::current()).line() + 1;
							hr = ExceptionToHRESULT(level, evt::TestEvent2, "mymessage");
							break;
						case EventType::kCharPtr:
							line = (location = source_location::current()).line() + 1;
							hr = ExceptionToHRESULT(level, "zstring event 2: {}: {}", "mymessage");
							break;
						case EventType::kString:
							line = (location = source_location::current()).line() + 1;
							hr = ExceptionToHRESULT(level, "string event 2: {}: {}", "mymessage");
							break;
						case EventType::kStringRef: {
							const std::string str = "string& event 2: {}: {}";
							line = (location = source_location::current()).line() + 1;
							hr = ExceptionToHRESULT(level, str, "mymessage");
							break;
						}
						default:
							throw std::out_of_range(fmt::format("eventType {}", eventType));
		...
		switch (eventType) {
		case EventType::kEvent:
			logOutput = GetLogEventOutput(!kHRESULT ? "Testing event 0: mymessage" : "Testing event 2: mymessage", dynamicLogMessage, location, line);
			break;
		case EventType::kCharPtr:
			logOutput = GetLogEventOutput(!kHRESULT ? "zstring event 0: mymessage" : "zstring event 2: mymessage", dynamicLogMessage, location, line);
			break;
		case EventType::kString:
			logOutput = GetLogEventOutput(!kHRESULT ? "string event 0: mymessage" : "string event 2: mymessage", dynamicLogMessage, location, line);
			break;
		case EventType::kStringRef:
			logOutput = GetLogEventOutput(!kHRESULT ? "string& event 0: mymessage" : "string& event 2: mymessage", dynamicLogMessage, location, line);
			break;
...
TEST_F(LogEvent_Test, Event_ErrorLogging_LogError) {
	...
	EXPECT_CALL(m_win32, EventWriteEx(t::_, t::Field(&EVENT_DESCRIPTOR::Id, evt::TestEvent0.Id), DTGM_ARG6))
		.InSequence(seq)
		.WillOnce(t::Return(ERROR_ACCESS_DENIED));

	EXPECT_CALL(m_win32, EventWriteEx(t::_,
	                                  t::AllOf(
										  t::Field(&EVENT_DESCRIPTOR::Id, evt::EventWrite.Id),
										  t::Field(&EVENT_DESCRIPTOR::Keyword, m4t::BitsSet(M3C_KEYWORD_EXCEPTION))),
	                                  DTGM_ARG6))
		.InSequence(seq);
	EXPECT_CALL(m_win32, EventWriteEx(t::_,
	                                  t::AllOf(
										  t::Field(&EVENT_DESCRIPTOR::Id, evt::Log_LogInternalError.Id),
										  t::Field(&EVENT_DESCRIPTOR::Keyword, m4t::BitsSet(M3C_KEYWORD_LOG))),
	                                  DTGM_ARG6))
		.InSequence(seq);

	Log::Critical(evt::TestEvent0, "mymessage");
}
#endif
...
#if _DEBUG
	EXPECT_CALL(m_win32, OutputDebugStringA(t::StartsWith(fmt::format("Error logging event {}:", evt::TestEvent0.Id))))
		.InSequence(seq);
#endif
...
#if M3C_LOG_OUTPUT_DEBUG || M3C_LOG_OUTPUT_STDERR
TEST_F(LogString_Test, LOG_TRACE_RESULT_int_Print) {
	if constexpr (kMinimumLevel >= Priority::kTrace) {
		const int result = LOG_TRACE_RESULT(7, "Result: {} - {}", "mymessage");
		EXPECT_EQ(7, result);

		if constexpr (kOutputDebug) {
			EXPECT_THAT(m_debug, m4t::MatchesRegex("\\[Trace\\] \\[\\d+\\] Result: 7 - mymessage\n\tat.*\n"));
		}
		if constexpr (kOutputStderr) {
...
#if M3C_LOG_OUTPUT_DEBUG || M3C_LOG_OUTPUT_STDERR
TEST_F(LogString_Test, LOG_TRACE_HRESULT_HRESULT_Print) {
	if constexpr (kMinimumLevel >= Priority::kTrace) {
		const HRESULT hr = LOG_TRACE_HRESULT(E_PENDING, "HRESULT: 0x{:X} - {}", "mymessage");
...
TEST_P(LogAll_Test, Exception_StdExceptionWithDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::TestEvent1>("", std::exception("myexception"), "Testing event 1: 3", evt::TestEvent1, 3);
}
...
TEST_P(LogAll_Test, Exception_StdSystemErrorAndPriorityIsEnabled_LogWithoutLocation) {
	TestMethod<evt::system_error>("", m4t::WithLocale("en-US", [] { return std::system_error(ERROR_ACCESS_DENIED, std::system_category()); }), "System error: Access is denied. (5)");
}

TEST_P(LogOutputs_Test, Exception_StdSystemErrorWithMessageAndPriorityIsEnabled_LogWithoutLocation) {
	TestMethod<evt::system_error>("", m4t::WithLocale("en-US", [] { return std::system_error(ERROR_ACCESS_DENIED, std::system_category(), "myexception"); }), "myexception: Access is denied. (5)");
}

TEST_P(LogAll_Test, Exception_StdSystemErrorWithLocationAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::system_error>("", m4t::WithLocale("en-US", [] { return std::system_error(ERROR_ACCESS_DENIED, std::system_category()); }), "System error: Access is denied. (5)", evt::Default);
}

TEST_P(LogOutputs_Test, Exception_StdSystemErrorWithMessageAndLocationAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::system_error>("", m4t::WithLocale("en-US", [] { return std::system_error(ERROR_ACCESS_DENIED, std::system_category(), "myexception"); }), "myexception: Access is denied. (5)", evt::Default);
}

TEST_P(LogAll_Test, Exception_StdSystemErrorWithDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::TestEvent1>("", m4t::WithLocale("en-US", [] { return std::system_error(ERROR_ACCESS_DENIED, std::system_category()); }), "Testing event 1: 3", evt::TestEvent1, 3);
}

TEST_P(LogOutputs_Test, Exception_StdSystemErrorWithMessageAndDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::TestEvent1>("", m4t::WithLocale("en-US", [] { return std::system_error(ERROR_ACCESS_DENIED, std::system_category(), "myexception"); }), "Testing event 1: 3", evt::TestEvent1, 3);
}

TEST_P(LogOutputs_Test, Exception_StdSystemErrorWithInfixMessageAndDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::system_error>("", m4t::WithLocale("en-US", [] { return std::filesystem::filesystem_error("myexception", "C:\\mypath.test", std::error_code(ERROR_ACCESS_DENIED, std::system_category())); }), "myexception: \"C:\\mypath.test\": Access is denied. (5)");
}

TEST_P(LogOutputs_Test, Exception_StdSystemErrorWithEmptyCustomMessageAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::system_error>("", m4t::WithLocale("en-US", [] { return custom_system_error(ERROR_ACCESS_DENIED, std::system_category(), ""); }), "System error: Access is denied. (5)");
}

TEST_P(LogOutputs_Test, Exception_StdSystemErrorWithCustomMessageAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::system_error>("", m4t::WithLocale("en-US", [] { return custom_system_error(ERROR_ACCESS_DENIED, std::system_category(), "myexception"); }), "myexception: Access is denied. (5)");
}

TEST_P(LogOutputs_Test, Exception_StdSystemErrorWithGenericCodeAndPriorityIsEnabled_LogWithoutLocation) {
	TestMethod<evt::std_system_error>("", m4t::WithLocale("en-US", [] { return std::system_error(std::make_error_code(std::errc::directory_not_empty)); }), "directory not empty");
}

TEST_P(LogOutputs_Test, Exception_StdSystemErrorWithGenericCodeAndMessageAndPriorityIsEnabled_LogWithoutLocation) {
	TestMethod<evt::std_system_error>("", m4t::WithLocale("en-US", [] { return std::system_error(std::make_error_code(std::errc::directory_not_empty), "myexception"); }), "myexception: directory not empty");
}
...
TEST_P(LogAll_Test, Exception_SystemErrorAndPriorityIsEnabled_LogWithoutLocation) {
	TestMethod<evt::system_error>("", system_error(ERROR_ACCESS_DENIED, std::system_category(), nullptr), "System error: Access is denied. (5)");
}

TEST_P(LogOutputs_Test, Exception_SystemErrorWithMessageAndPriorityIsEnabled_LogWithoutLocation) {
	TestMethod<evt::system_error>("", system_error(ERROR_ACCESS_DENIED, std::system_category(), "myexception"), "myexception: Access is denied. (5)");
}

TEST_P(LogAll_Test, Exception_SystemErrorWithLocationAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::system_error>("", system_error(ERROR_ACCESS_DENIED, std::system_category(), nullptr), "System error: Access is denied. (5)", evt::Default);
}

TEST_P(LogOutputs_Test, Exception_SystemErrorWithMessageAndLocationAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::system_error>("", system_error(ERROR_ACCESS_DENIED, std::system_category(), "myexception"), "myexception: Access is denied. (5)", evt::Default);
}

TEST_P(LogAll_Test, Exception_SystemErrorWithDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::TestEvent1>("", system_error(ERROR_ACCESS_DENIED, std::system_category(), nullptr), "Testing event 1: 3", evt::TestEvent1, 3);
}

TEST_P(LogOutputs_Test, Exception_SystemErrorWithMessageAndDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::TestEvent1>("", system_error(ERROR_ACCESS_DENIED, std::system_category(), "myexception"), "Testing event 1: 3", evt::TestEvent1, 3);
}
...
TEST_P(LogAll_Test, Exception_WindowsErrorAndPriorityIsEnabled_LogWithoutLocation) {
	TestMethod<evt::windows_error>("", windows_error(ERROR_ACCESS_DENIED), "Windows error: Access is denied. (5)");
}

TEST_P(LogOutputs_Test, Exception_WindowsErrorWithMessageAndPriorityIsEnabled_LogWithoutLocation) {
	TestMethod<evt::windows_error>("", windows_error(ERROR_ACCESS_DENIED, "myexception"), "myexception: Access is denied. (5)");
}

TEST_P(LogAll_Test, Exception_WindowsErrorLocationAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::windows_error>("", windows_error(ERROR_ACCESS_DENIED), "Windows error: Access is denied. (5)", evt::Default);
}

TEST_P(LogOutputs_Test, Exception_WindowsErrorWithMessageAndLocationAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::windows_error>("", windows_error(ERROR_ACCESS_DENIED, "myexception"), "myexception: Access is denied. (5)", evt::Default);
}

TEST_P(LogAll_Test, Exception_WindowsErrorWithDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::TestEvent1>("", windows_error(ERROR_ACCESS_DENIED), "Testing event 1: 3", evt::TestEvent1, 3);
}

TEST_P(LogOutputs_Test, Exception_WindowsErrorWithMessageAndDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::TestEvent1>("", windows_error(ERROR_ACCESS_DENIED, "myexception"), "Testing event 1: 3", evt::TestEvent1, 3);
}
...
TEST_P(LogAll_Test, Exception_RpcErrorAndPriorityIsEnabled_LogWithoutLocation) {
	TestMethod<evt::rpc_error>("", rpc_error(RPC_S_ADDRESS_ERROR), "RPC error: An addressing error occurred in the RPC server. (1768)");
}

TEST_P(LogOutputs_Test, Exception_RpcErrorWithMessageAndPriorityIsEnabled_LogWithoutLocation) {
	TestMethod<evt::rpc_error>("", rpc_error(RPC_S_ADDRESS_ERROR, "myexception"), "myexception: An addressing error occurred in the RPC server. (1768)");
}

TEST_P(LogAll_Test, Exception_RpcErrorWithLocationAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::rpc_error>("", rpc_error(RPC_S_ADDRESS_ERROR), "RPC error: An addressing error occurred in the RPC server. (1768)", evt::Default);
}

TEST_P(LogOutputs_Test, Exception_RpcErrorWithMessageAndLocationAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::rpc_error>("", rpc_error(RPC_S_ADDRESS_ERROR, "myexception"), "myexception: An addressing error occurred in the RPC server. (1768)", evt::Default);
}

TEST_P(LogAll_Test, Exception_RpcErrorWithDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::TestEvent1>("", rpc_error(RPC_S_ADDRESS_ERROR), "Testing event 1: 3", evt::TestEvent1, 3);
}

TEST_P(LogOutputs_Test, Exception_RpcErrorWithMessageAndDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::TestEvent1>("", rpc_error(RPC_S_ADDRESS_ERROR, "myexception"), "Testing event 1: 3", evt::TestEvent1, 3);
}
...
TEST_P(LogAll_Test, Exception_ComErrorAndPriorityIsEnabled_LogWithoutLocation) {
	TestMethod<evt::com_error>("", com_error(E_ABORT), "COM error: Operation aborted (0x80004004)");
}

TEST_P(LogOutputs_Test, Exception_ComErrorWithMessageAndPriorityIsEnabled_LogWithoutLocation) {
	TestMethod<evt::com_error>("", com_error(E_ABORT, "myexception"), "myexception: Operation aborted (0x80004004)");
}

TEST_P(LogAll_Test, Exception_ComErrorWithLocationAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::com_error>("", com_error(E_ABORT), "COM error: Operation aborted (0x80004004)", evt::Default);
}

TEST_P(LogOutputs_Test, Exception_ComErrorWithMessageAndLocationAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::com_error>("", com_error(E_ABORT, "myexception"), "myexception: Operation aborted (0x80004004)", evt::Default);
}

TEST_P(LogAll_Test, Exception_ComErrorWithDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::TestEvent1>("", com_error(E_ABORT), "Testing event 1: 3", evt::TestEvent1, 3);
}

TEST_P(LogOutputs_Test, Exception_ComErrorWithMessageAndDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::TestEvent1>("", com_error(E_ABORT, "myexception"), "Testing event 1: 3", evt::TestEvent1, 3);
}
...
TEST_P(LogAll_Test, Exception_NestedExceptionWithDetailAndPriorityIsEnabled_LogWithLocation) {
	TestMethod<evt::TestEvent1, true>("", std::exception("myexception"), "Testing event 1: 3", evt::TestEvent1, 3);
}
...
TEST_P(LogAll_Test, ExceptionToHRESULT_WindowsErrorAndPriorityIsEnabled_LogWithoutLocation) {
	const HRESULT hr = TestMethod<evt::windows_error, false, true>(
...
TEST_P(LogAll_Test, ExceptionToHRESULT_RpcErrorAndPriorityIsEnabled_LogWithoutLocation) {
	const HRESULT hr = TestMethod<evt::rpc_error, false, true>(
...
TEST_P(LogAll_Test, ExceptionToHRESULT_ComErrorAndPriorityIsEnabled_LogWithoutLocation) {
	const HRESULT hr = TestMethod<evt::com_error, false, true>(
...
#if M3C_LOG_OUTPUT_DEBUG

TEST_F(LogString_Test, Format_Reversed_PrintReversed) {
	Log::Critical(evt::FormatReversed, 3, 5, 7, L"Test-\u00FC");

	EXPECT_THAT(m_debug, t::HasSubstr("] Reversed: Test-\u00C3\u00BC; 7; 5; 3\n\tat"));
}

TEST_F(LogString_Test, Format_Width_PrintPadded) {
	Log::Critical(evt::FormatWidth, 3, 5, 7, L"Test");

	EXPECT_THAT(m_debug, t::HasSubstr("] Width:    3;     Test\n\tat"));
}

TEST_F(LogString_Test, Format_WidthLeft_PrintPaddedLeft) {
	Log::Critical(evt::FormatWidthLeft, 3, 5, 7, L"Test");

	EXPECT_THAT(m_debug, t::HasSubstr("] Width left: 3   ; Test    \n\tat"));
}

TEST_F(LogString_Test, Format_WidthZeroPadded_PrintZeroPadded) {
	Log::Critical(evt::FormatWidthZeroPadded, 3, 5, 7, L"Test");

	EXPECT_THAT(m_debug, t::HasSubstr("] Width zero padded: 0003\n\tat"));
}

TEST_F(LogString_Test, Format_WidthOPrecision_PrintTruncated) {
	Log::Critical(evt::FormatPrecision, 3, 5, 7, L"Test");

	EXPECT_THAT(m_debug, t::HasSubstr("] Precision: Te\n\tat"));
}

TEST_F(LogString_Test, Format_DynamicWidth_PrintPadded) {
	Log::Critical(evt::FormatDynamicWidth, 3, 5, 7, L"Test");

	EXPECT_THAT(m_debug, t::HasSubstr("] Dynamic width:   5\n\tat"));
}

TEST_F(LogString_Test, Format_DynamicWidthZeroPadded_PrintZeroPadded) {
	Log::Critical(evt::FormatDynamicWidthZeroPadded, 3, 5, 7, L"Test");

	EXPECT_THAT(m_debug, t::HasSubstr("] Dynamic width zero padded: 005\n\tat"));
}

TEST_F(LogString_Test, Format_DynamicPrecision_PrintTruncated) {
	Log::Critical(evt::FormatDynamicPrecision, 3, 5, 2, L"Test");

	EXPECT_THAT(m_debug, t::HasSubstr("] Dynamic precision: Te\n\tat"));
}

TEST_F(LogString_Test, Format_DynamicWidthAndPrecision_PrintPaddedAndTruncated) {
	Log::Critical(evt::FormatDynamicWidthAndPrecision, 3, 5, 2, L"Test");

	EXPECT_THAT(m_debug, t::HasSubstr("] Dynamic width and precision:    Te\n\tat"));
}

TEST_F(LogString_Test, Format_DynamicWidthLeftAndPrecision_PrintPaddedLeftAndTruncated) {
	Log::Critical(evt::FormatDynamicWidthLeftAndPrecision, 3, 5, 2, L"Test");

	EXPECT_THAT(m_debug, t::HasSubstr("] Dynamic width left and precision: Te   \n\tat"));
}

TEST_F(LogString_Test, Format_SpecialCharacters_PrintSpecialCharacters) {
	Log::Critical(evt::FormatSpecialCharacters, 3, 5, 7, L"Test");
